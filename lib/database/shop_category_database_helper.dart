...
class ShopCategoryDatabaseHelper {
  ...
  Future<int> updateShopCategory(ShopCategory shopCategory) async {
    final db = await AppDatabase().database;
    return await db.update(tableName, shopCategory.toMap(), where: 'shop_cat_id = ?', whereArgs: [shopCategory.shopCatId]);
  }

  Future<int> deleteShopCategory(int shopCatId) async {
    final db = await AppDatabase().database;
    return await db.delete(tableName, where: 'shop_cat_id = ?', whereArgs: [shopCatId]);
  }
}
